...
export default function AgentRequestsPage() {
    ...
    const fetchRequests = async (isPoll = false) => {
        ...
        try {
            const { data } = await axios.get(`/api/agent/requests?status=${statusFilter}&limit=50`)
            setRequests(data.requests || [])
            setLastRefreshed(new Date())
        } catch (err) {
            console.error("[AgentRequests]", err)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleProcess = async (req) => {
        ...
        try {
            await axios.post("/api/agent/cash-in", { agentRef: req.agentRef })
            toast.success(`Credited $${req.amount.toFixed(2)} to ${req.user?.name || "user"}'s wallet.`)
            fetchRequests()
        } catch (err) {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-slate-800">Top-Up Requests</h1>
                <p className="text-sm text-slate-500 mt-1">View and process all deposit requests</p>
            </div>
            ...
            <section className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                {loading ? (
                    ...
                    <div className="py-16 text-center">
                        <Filter size={36} className="mx-auto text-slate-200 mb-3" />
                        <p className="text-sm text-slate-400">
                            {searchRef ? "No requests match your search." : `No ${statusFilter} requests.`}
                        </p>
                    </div>
                ) : (
