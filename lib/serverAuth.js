...
export async function getVerifiedUserFromRequest() {
    ...
    try {
        const user = await prisma.user.findUnique({
            // Soft-deleted (closed) accounts can never act on the platform.
            where: { id: userId, deletedAt: null },
            // email + name are needed to populate the payment provider's
            // customer object (Flutterwave requires an email on checkout).
