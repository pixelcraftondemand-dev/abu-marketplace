...
export function logPayment(fields) {
  ...
  // Console output is dev-only — production stays silent rather than spamming
  // stdout (structured event logging belongs in an aggregator; for now Sentry
  // below is the production signal).
  ...
  // blocks or breaks the payment path). Safe fields only. Runs in ALL
  // environments — this is the production-visible payment log.
