...
//
// Provider interface (optional — used when an external payment provider exists):
 ...
 */
export async function reconcilePayment({ paymentId, prisma, provider = null }) {
  ...
  // Mark stuck payments as failed if they've been pending too long (24h).
  if (!provider) {
    ...
    if (stuckFor24h) {
      ...
      if (transition.applied) {
        await releaseStock(prisma, payment.orders.flatMap((o) => o.orderItems));
