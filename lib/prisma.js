...
if (!globalForPrisma.__prismaLogHandlersInstalled) {
  ...
  prisma.$on('error', (e) => {
    if (isExpectedRateLimitP2002(e)) {
      // Expected race — log one concise line, no stack. Count it so the
      // smoke battery can tell the filtered path is working (see
      // lib/prismaErrorCounters.js + check #12 in scripts/prod-smoke.sh).
      recordSuppressedRateLimitP2002();
