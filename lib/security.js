...
// Subscription checkout creates real Flutterwave hosted payments — limit per user.
...
// Blue-collar service hiring requests notify the team by email per request —
// generous enough for real lead flow, tight enough to stop spam flooding the inbox.
export const serviceRequestRateLimiter = createDistributedRateLimiter({ windowMs: 10 * 60_000, max: 5, name: "service-request" });
// Worker self-listings also email the team and should be rare per account.
export const serviceRegisterRateLimiter = createDistributedRateLimiter({ windowMs: 10 * 60_000, max: 3, name: "service-register" });
