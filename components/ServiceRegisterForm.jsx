...
const EMPTY = { name: "", phone: "", email: "", trade: "", location: "", experience: "", hourlyRate: "", bio: "" };
...
export default function ServiceRegisterForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post("/api/services/register", form);
      if (data.success) {
        setStatus({ state: "success", message: t("services.registerSuccess") });
        setForm(EMPTY);
      }
    } catch (error) {
      setStatus({
        state: "error",
        message: error.response?.data?.error || t("services.registerError"),
      });
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" aria-label={t("services.registerTitle")}>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input required type="email" value={form.email} onChange={set("email")} className={inputClass} placeholder="you@example.com" maxLength={160} />
        </div>
        <div>
          <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-[#6B6560]">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div>
          <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-[#6B6560]">
            {t("services.formExperience")}
          </label>
          <input value={form.experience} onChange={set("experience")} className={inputClass} placeholder="e.g. 5 years" maxLength={60} />
        </div>
        <div>
          <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-[#6B6560]">
            {t("services.formHourlyRate")}
          </label>
          <input value={form.hourlyRate} onChange={set("hourlyRate")} className={inputClass} placeholder="e.g. $8" maxLength={40} />
        </div>
      </div>
      ...
      <button
        ...
        className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#1A1A1A] px-6 py-3 text-sm font-semibold uppercase tracking-wide text-white transition hover:bg-[#2D2D2D] disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
      >
        {status.state === "loading" ? <Loader2 size={16} className="animate-spin" /> : <Hammer size={16} />}
        {t("services.submitListing")}
      </button>
    </form>
  );
}
